...
export class Logger {
  ...
  static debug(msg: string, ...params: any[]) {
    if (Logger.level >= LoggerLevels.DEBUG) {
      console.log.apply(console, ['%c' + msg, 'background: #d7ffd6; color: #014001;' + Logger.boldStyle, ...params.map(param => objectToString(param))])
    }
  }

  static debugError(msg: string, ...params: any[]) {
    if (Logger.level >= LoggerLevels.DEBUG) {
      console.log.apply(console, ['%c' + msg, 'background: #ffc2c2; color: #550000;' + Logger.boldStyle, ...params.map(param => objectToString(param))])
    }
  }

  static trace(msg: string, ...params: any[]) {
    if (Logger.level >= LoggerLevels.TRACE) {
      console.log.apply(console, ['%c' + msg, 'background: #e010e0; color: #240524;' + Logger.boldStyle, ...params.map(param => objectToString(param))])
    }
  }
}
